...
class CommentBloc extends Bloc<CommentEvent, CommentState>{
  ...
  CommentBloc():super(const CommentState()){
    on<CommentFetchedEvent>((event,emit) async {
      int totalItem = state.comments.length;
      try{
        if(state.status == CommentStatus.initial){
          final comments = await getCommentsFromApi(0, NUMBER_OF_COMMENTS_PER_PAGE);
          emit(state.copyWith(comments: comments, status: CommentStatus.success, hasReachedEnd: false));
        }else{
          final comments = await getCommentsFromApi(totalItem, NUMBER_OF_COMMENTS_PER_PAGE);
          if(comments.isEmpty){
            emit(state.copyWith(hasReachedEnd: true));
          }else{
            emit(state.copyWith(comments: List.of(state.comments)..addAll(comments), status: CommentStatus.success, hasReachedEnd: false));
          }
        }
      }catch(e){
